...
local killsays = { -- found by cheese
    ...
    "crosszay on yt <-- join for cool scripts (!!! real no fake !!!)",
    "ez bozo",
    ...
    "noobsploit better than you smh smh smh 😅"
}
...
menu = library.new('<font color="rgb(38, 55, 189)">noobsploit</font>ontop', "noobsploitcw")
local tabs = {
...
do
    ...
    local function update_cfgs()
        all_cfgs = listfiles("noobsploitcw\\")
        for _, cfg in next, all_cfgs do
            all_cfgs[_] = string.gsub(string.gsub(cfg, "noobsploitcw\\", ""), ".txt", "")
...
Notification:Notify(
    {Title = "Nofitication", Description = "script Loaded in " .. tick() - t .. "s (seconds)"},
    {OutlineColor = Color3.fromRGB(80, 80, 80), Time = 5, Type = "default"}
)
